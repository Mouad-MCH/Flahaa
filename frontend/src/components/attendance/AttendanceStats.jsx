...
const AttendanceStats = ({state : {totalWorkers, totalPresent, notRecorded}}) => {
  return (
    <div className='mb-4 grid grid-cols-3 gap-2 max-sm:grid-cols-1'>
      ...
      <StatCard label="Total workers" sub={'of 10 ative'} value={totalPresent} tone="present" />
      <StatCard label="Total workers" sub={'Mark attedance below'} value={notRecorded} tone="" />
    </div>
  )
}
