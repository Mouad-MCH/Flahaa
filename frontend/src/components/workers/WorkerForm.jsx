...
const WorkerForm = ({ worker, onSubmit, onClose, isSubmitting }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.CIN || form.CIN.trim().length < 3) nextErrors.CIN = 'CIN must be at least 3 characters';
    if (!form.daily_rate || Number(form.daily_rate) <= 0) nextErrors.daily_rate = 'Daily rate must be greater than 0';
    ...
    >
      <form id={FORM_ID} onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Full name" required error={errors.name}>
          <input type="text" className="input" value={form.name} onChange={handleChange('name')} placeholder="Ahmed Bennani" />
        </Field>

        <Field label="CIN" required error={errors.CIN}>
          <input type="text" className="input" value={form.CIN} onChange={handleChange('CIN')} placeholder="AB123456" />
        </Field>

        <Field label="Phone" hint="Optional">
          <input type="tel" className="input" value={form.phone} onChange={handleChange('phone')} placeholder="06 12 34 56 78" />
        </Field>

        <Field label="Address" hint="Optional">
          <input type="text" className="input" value={form.address} onChange={handleChange('address')} />
        </Field>

        <Field label="Contract type">
          <select className="select" value={form.contract_type} onChange={handleChange('contract_type')}>
        ...
        <Field label="Daily rate (MAD)" required error={errors.daily_rate}>
          <input type="number" min="0" step="0.01" className="input" value={form.daily_rate} onChange={handleChange('daily_rate')} />
        </Field>
        ...
        <Field label="Join date" hint="Optional">
          <input type="date" className="input" value={form.join_date} onChange={handleChange('join_date')} />
        </Field>
